...
PathTrie* PathTrie::get_path_vec(std::vector<int>& output, std::vector<int>& spaces) {
  return get_path_vec(output, spaces, ROOT_);
}
...
PathTrie* PathTrie::get_path_vec(std::vector<int>& output, std::vector<int>& spaces,
  ...
  if (character == stop || character == ROOT_ || output.size() == max_steps) {
    ...
    output.push_back(character);
    spaces.push_back(space_idx);
    return parent->get_path_vec(output, spaces, stop, max_steps);
  }
}
