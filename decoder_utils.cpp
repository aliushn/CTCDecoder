...
std::vector<std::pair<std::string, std::string>> get_beam_search_result(
  ...
  std::vector<std::pair<std::string, std::string>> output_vecs;
  for (size_t i = 0; i < beam_size && i < space_prefixes.size(); ++i) {
    std::vector<int> output;
    std::vector<int> spaces;
    space_prefixes[i]->get_path_vec(output, spaces);
    ...
    for (size_t j = 0; j < output.size(); j++) {
      ...
      output_str += ch;
      if(ch==" ")
        space_str += std::to_string(spaces[j])+"#";
    }
    // for (size_t j = 0; j < spaces.size(); j++) {
    //   int sp_idx = spaces[j];
    //   if (sp_idx!=-1)
    //     space_str += std::to_string(sp_idx)+"#";
    // }
    std::pair<std::string, std::string> output_pair(space_str,
                                               output_str);
